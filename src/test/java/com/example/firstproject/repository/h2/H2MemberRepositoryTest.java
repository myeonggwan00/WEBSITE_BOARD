...
class H2MemberRepositoryTest {
    ...
    void crud() throws SQLException {
        ...
        Member addMember = memberRepository.findById(member.getLoginId());
        log.info("addMember={}", addMember);

        Member findMember = memberRepository.findByNo(addMember.getId());
        log.info("findMember={}", findMember);
        assertThat(findMember).isEqualTo(addMember);

        memberRepository.update(new Member("update", "updateTest", "tester"), findMember.getId());
        Member updateMember = memberRepository.findByNo(findMember.getId());
